...
const ColorPaletteModal = ({ navigation }) => {
  ...
  const handleSwitch = useCallback(
    (item, newValue) => {
      if (newValue) {
        setSelectedColors((value) => [item, ...value]);
      } else {
        setSelectedColors([
        ...
        renderItem={({ item, index }) => (
          <View style={styles.listItem}>
            ...
            <Switch
              value={
                !!selectedColors.find(
                  (dataItem) => dataItem.colorName == item.colorName
                )
              }
...
const styles = StyleSheet.create({
  ...
  viewContainer: { flex: 1, paddingHorizontal: 10, paddingVertical: 10 },
  textInput: { width: "60%", marginBottom: 10 },
  listItem: {
    padding: 10,
    flexDirection: "row",
